...
data = pd.DataFrame({"order":total,"player":player,"total value":salary,"draft year":d_year,"contract start year" : contract_start_year,"start or relief" : start_relief,"signed age": signed_age,"contract length" : contract_length},)
#----------------------------------------平均年薪-----------------------------------------------------------
data["avg"] = round(data["total value"].div(data["contract length"]))



data.to_csv(r'D:\data_analysis_ML\v2.csv', index=False)
